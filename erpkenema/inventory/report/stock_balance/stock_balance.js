...
function getUserBranch(user_id) {
	return new Promise(function (resolve, reject) {
		frappe.call({
			method: "erpkenema.inventory.report.stock_balance.stock_balance.get_employee_branch",
			args: {
				user: user_id,
			},
...
function setDefaultBranchFilterValue() {
	getUserBranch(frappe.session.user)
		.then(function (userBranchValue) {
			const branchFilter = frappe.query_reports["Stock Balance"].filters.find(filter => filter.fieldname === "branch");
			if (userBranchValue) {
		...
		.catch(function (error) {
			...
			frappe.query_reports["Stock Balance"].filters.find(filter => filter.fieldname === "branch").default = null;
			frappe.query_reports["Stock Balance"].filters.find(filter => filter.fieldname === "branch").read_only = 0;
		});
}
